...
export default function Example() {
  return (
    <>
            ...
            <Menu.Items className="absolute right-0 w-56 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
              <div className="px-1 py-1 ">
                ...
                <Menu.Item>
                  {({ active }) => (
                    <button className={`${active ? "bg-purple-500 text-white" : "text-gray-900"} group flex rounded-md items-center w-full px-2 py-2 text-sm`}>
                      {active ? <DuplicateActiveIcon className="w-5 h-5 mr-2" aria-hidden="true" /> : <DuplicateInactiveIcon className="w-5 h-5 mr-2" aria-hidden="true" />}
                      Duplicate
                    </button>
                  )}
                </Menu.Item>
              ...
              <div className="px-1 py-1">
                <Menu.Item>
                  {({ active }) => (
                    <button className={`${active ? "bg-purple-500 text-white" : "text-gray-900"} group flex rounded-md items-center w-full px-2 py-2 text-sm`}>
                      {active ? <ArchiveActiveIcon className="w-5 h-5 mr-2" aria-hidden="true" /> : <ArchiveInactiveIcon className="w-5 h-5 mr-2" aria-hidden="true" />}
                      Archive
                    </button>
                  )}
                </Menu.Item>
                <Menu.Item>
                  {({ active }) => (
                    <button className={`${active ? "bg-purple-500 text-white" : "text-gray-900"} group flex rounded-md items-center w-full px-2 py-2 text-sm`}>
                      {active ? <MoveActiveIcon className="w-5 h-5 mr-2" aria-hidden="true" /> : <MoveInactiveIcon className="w-5 h-5 mr-2" aria-hidden="true" />}
                      Move
                    </button>
                  )}
                </Menu.Item>
      ...
      <div class="absolute w-96 top-5 right-5 bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
        <div class="flex">
          <div class="py-1">
          ...
          <div>
            ...
            <p class="text-sm">Votre compte à été bien créer.</p>
          </div>
        </div>
      </div>
    </>
